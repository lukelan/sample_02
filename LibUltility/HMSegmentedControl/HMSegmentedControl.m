...
- (id)initWithSectionTitles:(NSArray *)sectiontitles {
    ...
    if (self) {
        ...
        self.pointList = [[[NSMutableArray alloc] init] autorelease];
        
        int curX = 10;
        [self.pointList removeAllObjects];
        CGFloat stringWidth;
        for (NSString* item in self.sectionTitles) {
            [self.pointList addObject:[NSNumber numberWithInt:curX]];
            stringWidth = roundf([item sizeWithFont:self.font].width);
//            NSLog(@"wid init \"%@\": %f", item, stringWidth);
            curX += (stringWidth + 30);
        }
        [self.pointList addObject:[NSNumber numberWithInt:curX]];
...
//    self.font = [UIFont fontWithName:@"STHeitiSC-Light" size:18.0f];
    self.font = [UIFont getFontBoldSize16];
...
//            [self.pointList addObject:[NSNumber numberWithInt:curX]];
            ...
            CGFloat stringWidth = self.segmentWidth;
            if (self.type == HMSegmentedControlTypeText) {
                stringWidth = roundf([titleString sizeWithFont:self.font].width);
            }
//            NSLog(@"wid \"%@\": %f", titleString, stringWidth);
            CGFloat y = roundf(((self.height - self.selectionIndicatorHeight) / 2) + (self.selectionIndicatorHeight - stringHeight / 2)) - 2;
//            CGRect rect = CGRectMake(self.segmentWidth * idx, y, self.segmentWidth, stringHeight);
            CGRect rect = CGRectMake(curX, y, stringWidth, stringHeight);
            curX += (stringWidth + 30);
...
//            [titleLayer setFont:(__bridge CFTypeRef)(self.font.fontName)];
            ...
            {
                UIView *view = [self viewWithTag:TAG_VERTICAL_LINE];
                if (!view || !view.superview) {
                    UILabel* verticalLine = [[UILabel alloc] initWithFrame:CGRectMake(titleLayer.frame.origin.x + titleLayer.frame.size.width + 15, titleLayer.frame.origin.y + 5, 1, titleLayer.frame.size.height/2)];
                    verticalLine.backgroundColor = [UIColor colorWithWhite:0.7 alpha:1.0];
                    verticalLine.tag = TAG_VERTICAL_LINE;
                    [self addSubview:verticalLine];
                    [verticalLine release];
                } else {
                    [view setFrame:CGRectMake(titleLayer.frame.origin.x + titleLayer.frame.size.width + 15, titleLayer.frame.origin.y + 5, 1, titleLayer.frame.size.height/2)];
                }
            }
        }];
//        [self.pointList addObject:[NSNumber numberWithInt:curX]];
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.selectionLocation == HMSegmentedControlSelectionLocationDown)
        indicatorYOffset = self.bounds.size.height - self.selectionIndicatorHeight - self.selectionIndicatorHeight;
...
//    }
    ...
    int x = [[self.pointList objectAtIndex:self.selectedSegmentIndex] intValue];
    CGFloat width = [[self.pointList objectAtIndex:self.selectedSegmentIndex + 1] intValue] - [[self.pointList objectAtIndex:self.selectedSegmentIndex] intValue] - 30;
    if (self.selectionStyle == HMSegmentedControlSelectionStyleTextWidthStrip){
        return CGRectMake(x, indicatorYOffset, width, self.selectionIndicatorHeight);
    }
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(self.bounds, touchLocation))
    {
        ...
        int points = 0;
        int pointe = 0;
        for (int i=0; i<self.pointList.count - 1; i++) {
            points = [[self.pointList objectAtIndex:i] intValue];
            pointe = [[self.pointList objectAtIndex:i+1] intValue];
            if (touchLocation.x >= points && touchLocation.x < pointe) {
                segment = i;
                break;
            }
        }
...
- (void)setSelectedSegmentIndex:(NSInteger)index {
    [self setSelectedSegmentIndex:index animated:YES notify:NO];
}
...
- (void)setSelectedSegmentIndex:(NSUInteger)index animated:(BOOL)animated notify:(BOOL)notify {
    ...
    if ([view isMemberOfClass:[UIScrollView class]]){
        if (index == 1) {
            [view setContentOffset:CGPointMake(0, self.bounds.origin.y) animated:YES];
        }
    }  
