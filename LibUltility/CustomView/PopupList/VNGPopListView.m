...
#import "VNGPopListView.h"
#import "VNGPopListViewCell.h"

#define POPLISTVIEW_SCREENINSET 60.
#define POPLISTVIEW_HEADER_HEIGHT 50.
#define RADIUS 5.

@interface VNGPopListView (private)
- (void)fadeIn;
- (void)fadeOut;
@end

@implementation VNGPopListView {
...
- (id)initWithTitle:(NSString *)aTitle
    ...
    if(self = [self initWithTitle:aTitle options:aOptions height:aHeight])
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell)
        cell = [[VNGPopListViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentity];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([_delegate respondsToSelector:@selector(VNGPopListView:didSelectedIndex:)])
        [_delegate VNGPopListView:self didSelectedIndex:[indexPath row]];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if ([_delegate respondsToSelector:@selector(VNGPopListViewDidCancel)])
        [_delegate VNGPopListViewDidCancel];
