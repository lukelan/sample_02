...
// 戻り値：エンコード成功時=QRTRUE、データなし、または容量オーバー時=QRFALSE

QRBOOL CQR_Encode::EncodeData(int nLevel, int nVersion, QRBOOL bAutoExtent, int nMaskingNo, LPCSTR lpsSource, int ncSource)
{
	int i, j;
...
QRBOOL CQR_Encode::EncodeSourceData(LPCSTR lpsSource, int ncLength, int nVerGroup)
{
...
QRBOOL CQR_Encode::IsNumeralData(unsigned char c)
{
	if (c >= '0' && c <= '9')
		return QRTRUE;
...
QRBOOL CQR_Encode::IsAlphabetData(unsigned char c)
{
	if (c >= '0' && c <= '9')
		return QRTRUE;
...
QRBOOL CQR_Encode::IsKanjiData(unsigned char c1, unsigned char c2)
{
