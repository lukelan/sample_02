...
- (void)requestStarted
{
	...
	if ([self delegate] && [[self delegate] respondsToSelector:didStartSelector]) {
		[[self delegate] performSelector:didStartSelector withObject:self];
	}
	#if NS_BLOCKS_AVAILABLE
	if(startedBlock){
		startedBlock();
...
- (void)requestReceivedResponseHeaders:(NSMutableDictionary *)newResponseHeaders
{
	...
	if ([self delegate] && [[self delegate] respondsToSelector:didReceiveResponseHeadersSelector]) {
		[[self delegate] performSelector:didReceiveResponseHeadersSelector withObject:self withObject:newResponseHeaders];
	}
...
- (void)requestWillRedirectToURL:(NSURL *)newURL
{
	...
	if ([self delegate] && [[self delegate] respondsToSelector:willRedirectSelector]) {
		[[self delegate] performSelector:willRedirectSelector withObject:self withObject:newURL];
	}
...
- (void)reportFailure
{
	if ([self delegate] && [[self delegate] respondsToSelector:didFailSelector]) {
		[[self delegate] performSelector:didFailSelector withObject:self];
	}
...
{
	if ([self delegate] && [[self delegate] respondsToSelector:didReceiveDataSelector]) {
		[[self delegate] performSelector:didReceiveDataSelector withObject:self withObject:data];
	}
...
- (void)handleBytesAvailable
{
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		if ([self delegate] && [[self delegate] respondsToSelector:[self didReceiveDataSelector]]) {
