...
+ (id)sharedManagerWithCachePath:(NSString *)cachePath
{
    dispatch_once(&once, ^{instance = [[self alloc] initWithCachePath:cachePath];});
    return instance;
}
...
- (id)initWithCachePath:(NSString *)cachePath
{
    if ((self = [super init]))
    {
        _imageCache = [self createCacheWithCachePath:cachePath];
        _imageDownloader = SDWebImageDownloader.new;
        _failedURLs = NSMutableArray.new;
        _runningOperations = NSMutableArray.new;
    }
...
- (SDImageCache *)createCacheWithCachePath: (NSString *)cachePath
{
    return [SDImageCache sharedImageCacheWithCachePath:cachePath];
}
