...
+ (SDImageCache *)sharedImageCacheWithCachePath: (NSString *)cachePath
{
    dispatch_once(&once, ^{instance = [[self alloc] initWithNamespace:@"default" cachePath:cachePath];});
    return instance;
}
...
{
    return [self initWithNamespace:ns cachePath:nil];
}

- (id)initWithNamespace:(NSString *)ns cachePath: (NSString *)cachePath
{
    if ((self = [super init]))
    {
        ...
        {
            _diskCachePath = [NSString stringWithString:cachePath];
        }
#if TARGET_OS_IPHONE
